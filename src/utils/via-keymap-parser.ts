...
function resolveKeycode(keycode: string): string | null {
  ...
  // KC_NO, KC_TRNS
  if (keycode === "KC_NO" || keycode === "KC_TRNS" || keycode === "_______")
    return null;
...
function extractModifier(
  ...
  // 単独修飾キー
  const standaloneModMap: Record<
    string,
    { modifier: ModifierKeyInfo["modifier"]; side: string }
  > = {
  ...
  // MT(MOD_xxx, KC_xxx) - Mod-Tap
  const mtMatch = keycode.match(
    /^MT\((MOD_[LR]?(?:SFT|CTL|ALT|GUI))[^,]*,\s*(KC_\w+)\)$/,
  );
  if (mtMatch) {
...
function parseModShorthand(
  shorthand: string,
): ModifierKeyInfo["modifier"] | null {
...
function extractLayerKey(
  ...
  if (moMatch) {
    const layer = parseInt(moMatch[1], 10);
